...
class Order:
    ...
    def add_item(self, burger: Burger): # This is missing  #done
        """
        Adds an item (like a Burger object) to the order.
        """
        # Complete here
        if burger.get_price() <= 0:
            print("Item price must be positive.")
        else:
            self.items.append({"name" : burger.get_name() , "price" : burger.get_price()})
            print (f"{burger.get_name()} added to order")

    def get_total(self)-> float: # This is missing   #done
